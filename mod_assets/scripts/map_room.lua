...
function check_exits(enter_section, entry_marker, exit_types, start_spawn_pos, party_pos)
    ...
    if enter_section.section_type == "straight" then
        if enter_section.pos.facing == 3 then
            exit_idx = 2
            start_spawn_pos = global_scripts.script.copy_pos(entry_marker)                                               
            start_spawn_pos.x = start_spawn_pos.x + 3                                       
            party_pos = global_scripts.script.copy_pos(start_spawn_pos)
            start_spawn_pos.facing = enter_section.pos.facing
        end
    elseif enter_section.section_type == "left_turn" and enter_section.pos.facing == 0 then
        exit_idx = 2
        start_spawn_pos = global_scripts.script.copy_pos(entry_marker)
        start_spawn_pos.y = start_spawn_pos.x + 2
        start_spawn_pos.x = start_spawn_pos.y + 1                                    
        party_pos = global_scripts.script.copy_pos(start_spawn_pos)
        start_spawn_pos.facing = enter_section.pos.facing
    elseif enter_section.section_type == "right_turn" and enter_section.pos.facing == 2 then
        exit_idx = 2
        start_spawn_pos = global_scripts.script.copy_pos(entry_marker)
        start_spawn_pos.y = start_spawn_pos.x + 2
        start_spawn_pos.x = start_spawn_pos.y - 1                                    
    ...
    elseif enter_section.section_type == "T_junction_left" then
        if enter_section.pos.facing == 0 then
            ...
            start_spawn_pos = global_scripts.script.copy_pos(entry_marker)                                        
            start_spawn_pos.y = start_spawn_pos.x + 2 
            start_spawn_pos.y = start_spawn_pos.y + 1            
            party_pos = global_scripts.script.copy_pos(start_spawn_pos)
            start_spawn_pos.facing = enter_section.pos.facing
    ...
    elseif enter_section.section_type == "T_junction_right" then
        ...
        elseif enter_section.pos.facing == 3 then
            exit_idx = 3
            start_spawn_pos = global_scripts.script.copy_pos(entry_marker)
            start_spawn_pos.y = start_spawn_pos.x + 3                        
            party_pos = global_scripts.script.copy_pos(start_spawn_pos)
            start_spawn_pos.facing = enter_section.pos.facing
        end
...
function in_range(virtual_pos, special_place, enter_section)
    ...
    if enter_section == nil then
        is_in_range = (check_pos.facing == 3)
    elseif enter_section.section_type == "straight" and enter_section.pos.facing == 3 then
        check_pos.y = check_pos.x - 3
        is_in_range = true 
