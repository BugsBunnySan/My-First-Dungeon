...
stargate_addresses = {["XOOOOO"] = {level = 1, x = 11, y = 25, elevation = 0, facing=1, destination_gate="portal_1", destination_teleporter="portal_teleporter"},
                      ["OXOOXX"] = {level = 3, x = 4, y = 21, elevation = 0, facing=0, destination_gate=nil, destination_teleporter=nil}}
...
function check_address()
    ...
    if coordinates ~= nil then
        ...
        if stargate_last_coordinates.destination_gate ~= nil then
            global_scripts.script.deactivatePortal(findEntity(stargate_last_coordinates.destination_gate))
        end
        if stargate_last_coordinates.destination_teleporter ~= nil then
            findEntity(stargate_last_coordinates.destination_teleporter).controller:deactivate()    
        end        
    end
end